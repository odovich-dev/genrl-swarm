...
class GRPOLanguageTrainerModule(TrainerModule, LoggerMixin):
    ...
    def generate(
        ...
        if self.use_vllm:
            ...
            for _ in range(self.num_generations):
                ...
                if len(rollout) == 0:
                    [cite_start]rollout = [[comp] for comp in completions]
                    if return_completion_ids:
    ...
    def _process_inputs(self, inputs, with_template=True, for_training=False):
        ...
        if with_template:
            ...
            for item in inputs:
                ...
                [cite_start]count = self.num_generations if for_training else 1
            ...
            if for_training:
                ...
                for generations in inputs:
                    if isinstance(generations, list):
                        # Ensure we expand by the same factor as the prompt side
                        # If generations has fewer items than num_generations, repeat the last one
                        for i in range(self.num_generations):
                            if i < len(generations):
                                templated_prompts.append(generations[i])
                            else:
                                # If we don't have enough generations, repeat the last one
                                templated_prompts.append(generations[-1] if generations else "")
                    else:
                        # Single generation case - expand to match num_generations
                        for _ in range(self.num_generations):
                            templated_prompts.append(generations)
            else:
    ...
    def _initialize_model(self, enable_gradient_checkpointing):
        ...
        [cite_start]self.model = self.model.to(self.device)
    ...
    def _initialize_tokenizers(self):
        ...
        if self.processing_class.pad_token_id is None:
            [cite_start]self.processing_class.pad_token_id = self.processing_class.eos_token_id
    ...
    def _get_per_token_logps(self, model, input_ids, attention_mask, logits_to_keep):
        ...
        [cite_start]labels = input_ids[:, -logits_to_keep:].contiguous()
    ...
    def compute_loss(
        ...
        [cite_start]logits_to_keep = completion_ids.size(1)
        ...
        if self.beta != 0.0:
            ...
            per_token_kl = (
                [cite_start]torch.exp(ref_per_token_logps - per_token_logps)
                - (ref_per_token_logps - per_token_logps)
                - 1
            ) [cite: 49]

        [cite_start]advantages = inputs["advantages"]
        old_per_token_logps = (
    ...
    def step(
        ...
        [cite_start]assert stage_outputs is not None, f"No outputs found for stage {stage}"
        ...
        assert rewards_unmapped is not None, f"No rewards found for stage {stage}"

        rewards_2d = []
        for r in rewards_unmapped:
            ...
            elif isinstance(r, (list, tuple)): # Handles lists with multiple elements
                rewards_2d.append([1.0 + val for val in r[:self.num_generations]])
            else:
                # Fallback for unexpected types, potentially raise an error or handle as needed
                # For now, let's treat it as a zero reward with base for consistency
                rewards_2d.append([1.0] * self.num_generations) # Or [0.0] if no base desired

        rewards = torch.tensor(rewards_2d, dtype=torch.float32)
        ...
        with torch.no_grad():
            ...
            if rewards.shape[1] > 1:
                ...
                epsilon = 1e-6
                # Only normalize if standard deviation is meaningful
                if torch.all(reward_std > epsilon):
                    advantages /= reward_std + epsilon
                else:
                    # If std is too small, use a simple normalization or skip it
